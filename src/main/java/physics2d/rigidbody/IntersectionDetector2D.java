...
public class IntersectionDetector2D {
    ...
    public static boolean pointInCircle(Vector2f point, Circle circle) {
        ...
        // square rooting is an expensive operation, takes many cpu cycles
    ...
    public static boolean pointInBox2D(Vector2f point, Box2D box) {
        ...
        return pointLocalBoxSpace.x <= max.x && min.x <= pointLocalBoxSpace.x &&
                pointLocalBoxSpace.y <= max.y && min.y <= point.y;
    ...
    public static boolean lineAndCircle(Line2D line, Circle circle) {
        ...
        // simplifies to the ratio of projected line over actual line
        float t = centerToLineStart.dot(ab) / ab.lengthSquared();
    ...
    public static boolean raycast(Box2D box, Ray2D ray, RaycastResult result) {
        ...
        for (int i = 0; i < 2; i++) {
            if (JMath.compare(f.get(i), 0)) {
                // If the ray is parallel to the current axis,
                // and the origin of the ray is not inside, we have no hit
